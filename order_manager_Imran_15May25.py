...
class OrderManager:
        ...
        if current_gtt_pending_orders ==0 and current_pending_orders ==0:
            ...
            if ce_current_ltp is not None and pe_current_ltp is not None:
                if ce_current_ltp <= self.entry_value_data_dict['ce_entry_limit'] and pe_current_ltp <= self.entry_value_data_dict['pe_entry_limit']:
                    ...
                    self.logger.write(f"Condition is met PE Current LTP: {pe_current_ltp}  is less than {self.entry_value_data_dict['pe_entry_limit']} and CE Current LTP: {ce_current_ltp}  is less than {self.entry_value_data_dict['ce_entry_limit']}")
                    ...
                    self.logger.write(f"PE Current LTP: {pe_current_ltp}  is greater than {self.entry_value_data_dict['pe_entry_limit']}. Not placing the PE Order")
                    self.logger.write(f"CE Current LTP: {ce_current_ltp}  is greater than {self.entry_value_data_dict['ce_entry_limit']}. Not placing the CE Order")
            else:
                self.logger.write("⚠️ LTP Values are unable to fetch from the SmartAPI")
    ...
    def check_status_of_first_two_gtt_ordrers(self):
        ...
        ce_oder_rule_id = self.first_order_data_dict["ce_order1"]["rule_id"]
        pe_oder_rule_id = self.first_order_data_dict["pe_order1"]["rule_id"]
        ...
        for order in gtt_order_list_response['data']:
            if order.get('id') == ce_oder_rule_id:
                order_status = order.get('status') in ['triggered', 'complete']
                if order_status:
                    break

            if order.get('id') == pe_oder_rule_id:
                order_status = order.get('status') in ['triggered', 'complete']
                if order_status:
                    break
        return order_status
    ...
    def check_status_of_first_two_gtt_orders_in_orderbook(self):
        ...
        ce_oder_rule_id = self.first_order_data_dict["ce_order1"]["rule_id"]
        pe_oder_rule_id = self.first_order_data_dict["pe_order1"]["rule_id"]
        ce_unique_order_id= f"GTTV3_prod_{ce_oder_rule_id}"
        pe_unique_order_id = f"GTTV3_prod_{pe_oder_rule_id}"
            ...
            for order in todays_filtered_order_book_data:
                if order.get('uniqueorderid') == ce_unique_order_id or ce_unique_order_id in order.get('uniqueorderid'):
                    ...
                    if ce_order_status:
                        ...
                        cancel_gtt_order(self.smartApi,pe_oder_rule_id,self.entry_value_data_dict['pe_token'],self.logger)
                        ...
                        if len(self.first_success_order_data_dict) == 0:
                            self.first_success_order_data_dict = {"order_name":"ce_order1",
                             ...
                             "token":order["symboltoken"]}
                        order_status = True
                        self.logger.write(f"--------------First CE order status was success and cancelled PE order {pe_oder_rule_id}----------------------------")
                        break
                ...
                if order.get('uniqueorderid') == pe_unique_order_id or pe_unique_order_id in order.get('uniqueorderid'):
                    ...
                    if pe_order_status:
                        time.sleep(0.1)  ## need to modify
                        cancel_gtt_order(self.smartApi,ce_oder_rule_id,self.entry_value_data_dict['ce_token'],self.logger)
                        ...
                        if len(self.first_success_order_data_dict) == 0:
                            self.first_success_order_data_dict = {"order_name": "pe_order1",
                                                                  ...
                                                                  "token": order["symboltoken"]}
                        order_status = True
                        self.logger.write(f"--------------First PE order status was success and cancelled CE order {ce_oder_rule_id}----------------------------")
                        break
    ...
    def monitor_second_orders(self):
        ...
        while True:
            ...
            if datetime.now(pytz.timezone("Asia/Kolkata")) >= END_TIME:
                ...
                break  # Exit the loop
            try:
                self.logger.write(f"STEP4 in Orders: Monitoring second set of gtt order check status at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
                time.sleep(0.5)
                ...
                if gtt_order1_flag:
                    ...
                    if gtt_order1_status is None:  # Replace with 'is None' if needed
                ...
                if gtt_sl_order1_flag:
                    ...
                    if gtt_sl_order1_status is None:
    ...
    def monitor_third_orders(self):  ### Imran Thougghts :: CHECK IF ORDER IS ALIVE THEN PROCEED WITH MODIFICATIONS OTHERWISE BREAK
        ...
        while True:
            ...
            if datetime.now(pytz.timezone("Asia/Kolkata")) >= END_TIME:
                ...
                break  # Exit the loop
            try:
                self.logger.write(f"STEP5 in Orders: Monitoring Third set, GTT SL order2 and SL Order3 status at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
                time.sleep(0.5)
                ...
                if gtt_sl_order2_status is not None:
                    gtt_sl_order2_info_from_api = self.smartApi.gttDetails(rule_sl_id2)
                    gtt_sl_order2_status = gtt_sl_order2_info_from_api["data"]["status"]
                    qtty2 = gtt_sl_order2_info_from_api["data"]["qty"]

                if gtt_sl_order3_status is not None:
                    gtt_sl_order3_info_from_api = self.smartApi.gttDetails(rule_sl_id3)
                    gtt_sl_order3_status = gtt_sl_order3_info_from_api["data"]["status"]
                    qtty3 = gtt_sl_order3_info_from_api["data"]["qty"]

                self.logger.write(f"Current LTP value: {ltp_response} and assumption value "
                                  f"{assumption_value} and third assumption value "
                                  f"{assumption_third_order} and entered price * assumption value  {roundof(entered_price * assumption_value)}"
                                  f"SL Order3 Status: {gtt_sl_order3_status} and SL Order2 status: {gtt_sl_order2_status}")
                # ltp_response = 110
                if assumption_value == assumption_third_order and ltp_response >= entered_price * assumption_value:
                    ...
                    elif gtt_sl_order2_status is None and gtt_sl_order3_status is None:
                        self.logger.write(f"-----------BOTH SL Orders are executed so existing and restarting the orders-----")
                        order_status = True
                        break

                elif ltp_response >= entered_price * assumption_value and gtt_sl_order2_status is None and gtt_sl_order3_status is not None:   ## stars with 1.08 and goes on
    ...
    def main_order_monitoring_loop(self,mock_api=None):
        ...
        self.logger.write(f"******** Started orders Monitoring from main_order_monitoring_loop at {datetime.now(pytz.timezone('Asia/Kolkata'))} **********")
        ...
        while condition:
            try:
                ...
                if self.first_orders_placed and flow_control['placing_first_order']:
                    self.logger.write(f"STEP1 IN ORDERS: Placing First set of orders at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
                    ...
                    self.logger.write(f"STEP2 IN ORDERS: Monitoring first two orders gtt orderbook status at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
                    ...
                    self.logger.write(f"STEP3 IN ORDERS: Placing second set of gtt orders at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
                    ...
                    self.logger.write(f"STEP5 IN ORDERS: Monitoring third orders status at {datetime.now(pytz.timezone('Asia/Kolkata'))}...")
